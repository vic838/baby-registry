...
export default function AdminPage() {
  ...
  async function loadRows() {
    try {
      ...
      if (!res.ok) {
        throw new Error(json?.error || "Failed to load reported checkouts");
      }
  ...
  async function markPaid(checkoutId: string) {
    try {
      ...
      if (!res.ok) {
        throw new Error(json?.error || "Failed to mark checkout as paid");
      }
      ...
      <div className="mx-auto max-w-4xl space-y-6">
        <div>
          ...
          <p className="mt-1 text-sm text-neutral-600">
            Confirm received payments and mark them as paid.
          </p>
        </div>
